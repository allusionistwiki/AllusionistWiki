...
content/allusions/**/*.md
content/mysteries/**/*.md
content/analogies/**/*.md
...
- 上流 OSS のコード
- npm パッケージ
- **Quartz 本体（`quartz/` ディレクトリは上流リポジトリ jackyzha0/quartz のライセンスに従う）**
- 原作・出版社・権利者が管理する素材
- 画像、フォント、アイコン、商標、ロゴ
...
特に原作小説の本文、抜粋、テキストデータ、PDF、EPUB などが含まれる場合は、
...
- 感想・解釈・仮説（`content/reflections/` 解釈層）を共有・転載する
...
> 解釈層（`content/reflections/`）は事実ページとは異なる解釈層であり、
...
本リポジトリは非公式のファン解析 Wiki の Web 公開版です。
...
| レイヤー | 対象 | ライセンス |
|---|---|---|
| A | Wiki 本文・独自解説・要約・考察・編纂物（`content/**/*.md`） | CC BY-NC-SA 4.0 |
